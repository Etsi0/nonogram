...
export function draw() {
    ...
    for (let i = 0; i < MAX_HEALTH; i++) {
        ...
        ctx.drawImage(images[`Heart_${i <= current - 1 ? 'filled' : 'empty'}`], workarea.x + offset + PercentageToPixels(size.healthBar + size.healthBar / 2) * i, workarea.y, PercentageToPixels(size.healthBar), PercentageToPixels(size.healthBar));
    }
}
